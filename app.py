...
with tab_assets:
    ...
    with st.form("add_asset_form"):
        ...
        with col_t:
            new_ticker = st.text_input("티커 (예: SPY, IBIT)", placeholder="SPY").strip().upper()
        with col_n:
            new_name = st.text_input("자산 이름 (예: S&P500)", placeholder="S&P500").strip()
        with col_c:
            new_cat = st.selectbox("카테고리 분류", ["주식", "원자재", "채권", "크립토", "기타"])
        with col_b:
            st.write("")  # 수직 정렬용 여백
            st.write("")
            add_submitted = st.form_submit_button("➕ 자산 추가", use_container_width=True)
    ...
    if not df_assets.empty:
        ...
        with col_del_btn:
            st.write("")
            st.write("")
            delete_btn = st.button("🗑️ 선택 자산 삭제", use_container_width=True, type="primary")
...
with tab_settings:
    ...
    with col_ma_a:
        ma_a_val = st.number_input("이평선 A 기간 (일수)", min_value=5, max_value=500, value=config["ma_settings"].get("ma_a", 200))
    with col_ma_b:
        ma_b_val = st.number_input("이평선 B 기간 (일수)", min_value=5, max_value=500, value=config["ma_settings"].get("ma_b", 288))
    with col_ma_c:
        ma_c_val = st.number_input("이평선 C 기간 (일수)", min_value=5, max_value=500, value=config["ma_settings"].get("ma_c", 365))
        
    col_ma_t, col_cond = st.columns(2)
    with col_ma_t:
        ma_type_val = st.selectbox("이동평균선 계산 방식", ["SMA (단순)", "EMA (지수)"], 
                                   index=0 if config["ma_settings"].get("ma_type", "SMA") == "SMA" else 1)
    with col_cond:
        ...
        selected_cond_key = st.selectbox(
            ...
            index=0 if config["ma_settings"].get("condition_type", "joint_turn") == "joint_turn" else 1
        )
    ...
    smtp_server_val = st.text_input("SMTP 서버 주소", value=email_settings.get("smtp_server", "smtp.gmail.com"))
    smtp_port_val = st.number_input("SMTP 포트 번호", min_value=25, max_value=65535, value=email_settings.get("smtp_port", 587))
    sender_email_val = st.text_input("보내는 이메일 주소 (ID)", value=email_settings.get("sender_email", ""))
    sender_password_val = st.text_input("보내는 이메일 비밀번호 (앱 비밀번호)", type="password", value=email_settings.get("sender_password", ""))
    receiver_email_val = st.text_input("받는 이메일 주소", value=email_settings.get("receiver_email", ""))
    send_on_trigger_val = st.checkbox("🚨 대시보드 스캔 완료 시 이메일 즉시 자동 발송", value=email_settings.get("send_on_trigger", True))
    
    # 설정 저장 및 테스트 버튼
    col_save, col_test = st.columns(2)
    with col_save:
        save_settings = st.button("💾 설정 저장하기", use_container_width=True)
    with col_test:
